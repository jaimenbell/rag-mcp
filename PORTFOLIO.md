...
> mcp-factory manifest model. 23 tests, all green.
...
- `python -m pytest` -> **23 passed** (chunking 6, ingest 7, search 7, MCP server 3).
...
5. **(48-60s) It's a real MCP tool.** Show `pytest` 23 green + the `mcp.yaml` validating in the mcp-factory model.
