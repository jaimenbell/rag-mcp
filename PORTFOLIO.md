...
> mcp-factory manifest model. 251 tests, all green (2026-09-15).
...
- `python -m pytest` -> **251 passed** (2026-09-15, junitxml testcase count; the largest files: ingest 54, store 25, cli 16, search 14, lock 12, server 11; CI-enforced by `scripts/check_readme_counts.py`, which counts total collected because one WINDOWS_ONLY test skips on Linux). The 2026-06-29 entry read 53.
- Real ingest of a 12-file corpus -> **129 chunks** with the live ONNX embedder (measured 2026-06-29; not re-measured since).
...
5. **(48-60s) It's a real MCP tool.** Show `pytest` 251 green + the `mcp.yaml` validating in the mcp-factory model.
